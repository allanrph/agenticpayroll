...
async def calculate_payroll(
    ...
    calculate_payroll_use_case: CalculatePayrollUseCase = Depends(
        get_calculate_payroll_use_case
    ),
):
    return await calculate_payroll_use_case.execute(request)
...
async def get_payslip(record_id: str, db: AsyncSession = Depends(get_async_db)):
    ...
    return FileResponse(
        record.payslip_path, media_type="application/pdf", filename="payslip.pdf"
    )
